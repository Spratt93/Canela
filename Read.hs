...
expandObjectLists string = [subject ++ predicate ++ object | object <- splitList]
  where subject = (takeWhile (\x -> x /= '>') string) ++ ">"
        predicate = (takeWhile (\x -> x /= '>') (drop (length subject) string)) ++ ">"
        splitList = splitOnChar ',' (drop ((length subject) + (length predicate)) string) ""

--takes a predicateList and expands it into a list of the full triples that it describes
expandPredicateLists :: String -> [String]
expandPredicateLists string = [subject ++ predicateAndObject | predicateAndObject <- splitList]
  where subject = (takeWhile (\x -> x /= '>') string) ++ ">"
        splitList = splitOnChar ';' (drop (length subject) string) ""
